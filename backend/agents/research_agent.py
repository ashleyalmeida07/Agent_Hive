...
RESEARCH_SYSTEM_PROMPT = """You are AgentHive ResearchAgent — an expert AI researcher.

You have access to tools that create real files in a project workspace. USE THEM.

## How You Work
1. Analyze the research task.
2. Formulate a comprehensive research report, gathering all necessary information.
3. Save the report using the `create_file` tool (e.g. as `report.md`).
4. After creating the requested files, call `task_complete` with a summary.

## CRITICAL RULES
- You MUST use `create_file` to save your final research report.
- Ensure the report is highly detailed and properly formatted in Markdown.
- Call `task_complete` when finished.
"""


class ResearchAgent:
    """Runs a multi-turn tool-use loop tailored for research tasks."""
    ...
    def run(self, task: dict, task_id: str, on_event: Callable):
        ...
        category = task.get("category", task.get("task_type", "research"))
        ...
        messages = [
            ...
            {"role": "user", "content": (
                ...
                f"## Category: {category}\n"
                f"## Description:\n{description}\n\n"
                "Write a comprehensive research report based on the requirements. "
                "Save it using the create_file tool. Call task_complete when finished."
            )},
        ]
        ...
        while iteration < self.max_iterations and not task_done:
            ...
            try:
                ...
                on_event({"type": "error", "message": f"LLM API Error: {str(e)}"})
                break
        ...
        on_event({
            "type": "complete",
            "files": files,
            "summary": f"Completed research and created {len(files)} files",
